...
public class TeamsAcceptanceTest
{
    ...
    async Task should_be_able_to_create_team()
    {
        ...
        httpHandlerMock
            ...
            .SetupSequence<Task<HttpResponseMessage>>(
                "SendAsync",
                ItExpr.Is<HttpRequestMessage>(
                    m => m.Content != null && m.Method.Equals(HttpMethod.Post) && m.Content.Equals(teamJson) && m.RequestUri!.AbsoluteUri.Contains("/groups")),
                ItExpr.IsAny<CancellationToken>())
        ...
        Assert.Equal( $"{baseUrl}/group-content/{teamId}", navMan.Uri);
    ...
    async Task shouldBeAbleToNavigateToTeamsPageAndSeeItsComponents()
    {
        ...
        var targetPage = "groups-list-page";
